...
<div id="jssor_1"
	...
	<!-- Loading Screen -->
	<div data-u="loading"
		style="position: absolute; top: 0px; left: 0px; background: url('../../images/loading.gif') no-repeat 50% 50%; background-color: rgba(0, 0, 0, 0.7);"></div>
	<div data-u="slides"
...
<jstl:forEach items="${listYtId}" var="video">
	<iframe width="560" height="315"
		src="https://www.youtube.com/embed/${video}" allowfullscreen
...
<jstl:if test="${isMovie eq false }">
	<acme:display code="content.season" />
	<display:table pagesize="5" class="displaytag" name="seasons"
		...
		<display:column>
			<a href="season/display.do?seasonId=${row.id }"><spring:message
					code="misc.view" /></a>
		</display:column>
		<jstl:if test="${producerId eq content.producer.id}">
			<display:column>
				<acme:link href="season/producer/edit.do?seasonId=${row.id}"
					code="misc.edit" />
